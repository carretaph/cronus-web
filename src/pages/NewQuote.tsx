...
function calculateProductPrice(
  ...
  const squareFeet =
    width > 0 && height > 0 ? (width * height) / 144 : 0
  ...
  const basePrice = squareFeet * rate
  const impactPrice =
    opening.impact && !isDoor
      ? basePrice *
        (pricingConfig.impactWindowMultiplier - 1)
      : 0

  return basePrice + impactPrice
}
    ...
    return activeDiscounts.map((discount) => {
      ...
      const requestedAmount =
        discount.mode === 'percent'
          ? previousPrice * (discount.value / 100)
          : discount.value
  ...
  function handleSelectCustomer(customerId: string) {
    ...
    setProjectForm((currentForm) => ({
      ...
      projectName:
        ...
        `${customer.firstName} ${customer.lastName} Project`,
      projectAddress:
        currentForm.projectAddress || customerAddress,
    }))
  }
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              {(
                ...
                <button
                  ...
                  className={`min-h-16 rounded-xl border px-5 text-sm font-medium transition ${
                    disposition === 'Sale'
...
function FinancingCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`min-h-[210px] rounded-[22px] border p-6 text-left transition ${
        selected
